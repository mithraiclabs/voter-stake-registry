...
async fn test_accelerated_vesting() -> Result<(), TransportError> {
    ...
    let accelerate_vesting = |index: u8, periods: u32, kind: LockupKind| {
      // TODO: change to accelerate vesting
    ...
    accelerate_vesting(7, 2, LockupKind::Daily)
        .await
        .expect_err("can't relock for less periods");
      accelerate_vesting(7, 3, LockupKind::Daily).await.unwrap(); // just resets start to current timestamp
    assert_eq!(lockup_status(7).await, (0, 3 * day, 90, 90, 0));
    ...
    accelerate_vesting(7, 10, LockupKind::Daily).await.unwrap();
    ...
    accelerate_vesting(7, 5, LockupKind::Daily)
        .await
        .expect_err("can't relock for less periods");
      accelerate_vesting(7, 6, LockupKind::Daily).await.unwrap();
    assert_eq!(lockup_status(7).await, (0, 6 * day, 80, 80, 0));
    accelerate_vesting(7, 8, LockupKind::Daily).await.unwrap();
    ...
    accelerate_vesting(7, 1, LockupKind::Monthly).await.unwrap();
    assert_eq!(lockup_status(7).await, (0, 1 * month, 70, 70, 0));

    accelerate_vesting(7, 31, LockupKind::Daily)
        .await
        .expect_err("decreasing strictness");
    accelerate_vesting(7, 31, LockupKind::None)
        .await
        .expect_err("decreasing strictness");
    accelerate_vesting(7, 30, LockupKind::Cliff)
        .await
        .expect_err("period shortnend");
    accelerate_vesting(7, 31, LockupKind::Cliff).await.unwrap();
    ...
    accelerate_vesting(5, 2, LockupKind::Cliff)
        .await
        .expect_err("can't relock for less periods");
    accelerate_vesting(5, 3, LockupKind::Cliff).await.unwrap(); // just resets start to current timestamp
    assert_eq!(lockup_status(5).await, (0, 3 * day, 80, 80, 0));
    accelerate_vesting(5, 4, LockupKind::Cliff).await.unwrap();
    ...
    accelerate_vesting(5, 1, LockupKind::Cliff).await.unwrap();
